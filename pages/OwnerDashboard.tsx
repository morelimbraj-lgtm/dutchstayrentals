...
import { fetchTenantData, TenantMatch } from '../utils/googleSheetParser';
...
const OwnerDashboard: React.FC = () => {
  ...
  const [pendingPropertyData, setPendingPropertyData] = useState<Omit<Property, 'id' | 'ownerId' | 'createdAt'> | null>(null);
  const [pendingId, setPendingId] = useState<string | undefined>(undefined);
  const [isSaving, setIsSaving] = useState(false);

  // Tenant Data State
  const [tenantData, setTenantData] = useState<TenantMatch[]>([]);
  const [isLoadingTenantData, setIsLoadingTenantData] = useState(false);

  useEffect(() => {
    if (activeTab === 'tenantData' && tenantData.length === 0) {
  ...
  const handlePropertySubmit = (propertyData: Omit<Property, 'id' | 'ownerId' | 'createdAt'>, id?: string) => {
    setPendingPropertyData(propertyData);
    setPendingId(id);
    setIsSaveConfirmationOpen(true);
  };

  const confirmSaveProperty = async () => {
    if (!pendingPropertyData || !user) return;

    setIsSaving(true);
    try {
      if (pendingId) { // Editing existing property
        await updateProperty(pendingId, pendingPropertyData);
      } else { // Adding new property
        await addProperty(pendingPropertyData, user.id);
      }
  ...
  const renderTenantData = () => (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-semibold text-white">Tenant Matching Data</h2>
        <Button onClick={() => window.open('https://docs.google.com/spreadsheets/d/1KKlE1No1Se9YRcdEQZ2AkUUqr4U7IxY2Qmrt2UjopsA/edit?usp=sharing', '_blank')} variant="secondary" className="text-xs">
          View Source Sheet
        </Button>
      </div>
            ...
            <table className="min-w-full divide-y divide-white/10">
              <thead className="bg-white/5">
                <tr>
                  ...
                  <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Analysis</th>
                  <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Check</th>
                </tr>
              </thead>
                ...
                {tenantData.length > 0 ? (
                  tenantData.map((tenant, index) => (
                    <tr key={index} className="hover:bg-white/5 transition-colors">
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-400">#{tenant.matchId || index + 1}</td>
                      ...
                      <td className="px-6 py-4">
                        <div className="text-sm text-gray-300 line-clamp-3" title={tenant.reason}>{tenant.reason}</div>
                      </td>
                  ...
                  <tr>
                    <td colSpan={5} className="px-6 py-12 text-center text-gray-500">
                      No tenant matches found in the connected sheet.
                    </td>
                  </tr>
                )}
              </tbody>
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 pb-12 animate-fade-in-up">
      ...
      <div>
        <div className="border-b border-white/10 mb-8">
          <nav className="-mb-px flex space-x-8" aria-label="Tabs">
            ...
            <button onClick={() => setActiveTab('tenantData')} className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors ${activeTab === 'tenantData' ? 'border-indigo-500 text-indigo-400' : 'border-transparent text-gray-400 hover:text-gray-200 hover:border-gray-500'}`}>
              Tenant Data
            </button>
          </nav>
        </div>
      ...
      {/* Save Property Confirmation Modal */}
      <Modal
      ...
      >
        <div className="space-y-4">
          <div className="flex items-start gap-3 bg-indigo-500/10 p-4 rounded-lg border border-indigo-500/20">
            ...
            <div>
              <h3 className="text-indigo-300 font-semibold">Review Details</h3>
              <p className="text-indigo-200/70 text-sm mt-1">Please review the property details before confirming.</p>
            </div>
          </div>
          <p className="text-gray-300">
            Are you sure you want to {pendingId ? 'update' : 'add'} this property?
          </p>
          <div className="flex justify-end gap-3 pt-2">
            <Button variant="secondary" onClick={() => setIsSaveConfirmationOpen(false)}>Cancel</Button>
            <Button onClick={confirmSaveProperty} disabled={isSaving}>
